package makila.comparableentititymining.postagger;
...
public class StanfordPosTagger {
	...
	/**
	 * Splits the document into sentences, then splits each sentence into words after which POS tags are added.
	 * @param documentURL The document to be tagged
	 * @return A list containing a list of sentences containing tokenized words with associated POS tags
	 */
	...
	/**
	 * Splits the document into sentences, then splits each sentence into words after which POS tags are added.
	 * @param question The string to be tagged
	 * @return A list containing a list of sentences containing tokenized words with associated POS tags
	 */
	...
	/**
	 * Tags the words in the document for each sentence and return them as String representation
	 * @param documentURL The document to be tagged
	 ...
	 * @param question The string to be tagged
	 * @return A list containing POS tags generated from each word in the question
	 */
	public static List<List<String>> getStringTags(String question) {
		List<List<String>> tags = new ArrayList<List<String>>();
		List<List<HasWord>> sentences = MaxentTagger.tokenizeText(new StringReader(question));
		List<ArrayList<TaggedWord>> taggedWords = tagger.process(sentences);
	...
	/**
	 * Tokenizes the input question into sentences and tokenize each sentence into words.
	 * @param question The string containing a question to be tokenized.
	...
	/**
	 * Tags a string with it's POS tags and then returns a string representation of said string.
	 * @param question The string to be tagged
	...
	/**
	 * Given an input document specified by documentURL. Splits
	 * the document into sentences after which each sentence is split into
	 * word tokens. These words are then tagged with their POS tags after which
	 * only the POS tags are returned as a word list separated into sentences.
	 * @param documentURL The document URL for the text document to be read
	 * @return A double list split into sentences and words containing POS tags generated from the sentences in the document.
	 */
	...
	/**
	 * This one is similar to {@link #getTagsFromDocument(String)} but instead of returning only the
	 * POS tags, it only returns the words. It's effectively a sentence and word tokenizer.
	 * @see #getTagsFromDocument(String)
	 * @param documentURL The document to be tokenized
	 * @return The document found in documentURL tokenized into sentences and words.
	 */
	...
	/**
	 * Returns a string representation of a list containing a string. The 
	 * list is assumed to contain one sentence.
	 * @param tokenizedString The list containing words from one sentence
	...
	public static PosTag posTagString(String string) {
		...
		for(ArrayList<TaggedWord> list : taggedWords) {
			...
			while(it.hasNext()) {
				...
				else if(tag.value().equals("$") && tag.tag().equals("$") && it.hasNext()) {
		...
		PosTag posTag = new PosTag(taggedWords, PTBTokenizer.ptb2Text(sb.toString()));
		return posTag;
	}
