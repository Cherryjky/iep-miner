...
import makila.comparableentititymining.postagger.StanfordPosTagger.PosTag;
import edu.stanford.nlp.ling.TaggedWord;


public class MiningIndicativeExtractionPatterns {
	//alpha, beta, gamma, lambda parameters are determined empirically
	//alpha determined by maximizing precision without hurting recall by investigating frequencies of pairs in a labeled set
	/**
	...
	/**
	 * Constructor for this class. Initializes values and starts the bootstrapping method of extracting IEPs.
	 * @param iep The initial seed IEP
	 */
	...
	/**
	 * Does the actual work of extracting IEP through a bootstrapping approach. The bootstrapping process starts
	 * with a single IEP. From it, we extract a set of initial seed comparator pairs. For each comparator pair, 
	 * all questions containing the pair are retrieved from a question collection and regarded as comparative questions.
	 * From the comparative questions and comparator pairs, all possible sequential patterns are generated and evaluated by measuring
	 * their reliability score. Patterns evaluated as reliable ones are IEPs and are added into an IEP repository. 
	 * Then, new comparator pairs are extracted from the collection using the latest IEPs. The new comparators are added to a reliable
	 * comparator repository and used as new seeds for pattern learning in the next iteration. All questions from which
	 * reliable comparators are extracted are removed from the collection to allow finding new patters efficiently in 
	 * later iterations. The process iterates until no more new patterns can be found from the questions collection.
	 * The method has two key steps:
	 * <ol>
	...
	private List<String> iepMining() {
		...
		//	List<String> newComparativeQuestions = comparativeQuestionIdentify(seedComparatorPairs, questionArchive);
			...
			//newPatterns = PatternExtraction.mineGoodPatterns(seedComparatorPairs, comparativeQuestionSet);
			newComparatorPairs.clear();
			for(String q : questionArchive) {
				List<String> comparatorPairs = extractComparableComparators(iep, q);
	...
	/**
	 * For a given question, return true if the question matches any pattern in iep, else false.
	 * @param iep A list of extracted IEP patterns
	 * @param q A question that is compared with the IEPs
	...
	/**
	 * For a given set of questions and a set of seed comparator pairs, for each pair, if a text contains the pair it's regarded
	 * as a comparative question. 
	 * @param comparatorPairs The seed comparator pairs that are used to identify comparative questions
	 * @param questions A set of questions to be compared with the comparator pairs.
	 * @return All questions considered to be comparative questions with regard to the comparator pairs.
	 */
	@SuppressWarnings("unused")
	private List<String> comparativeQuestionIdentify(List<Pair<String, String>> comparatorPairs, String[] questions) {
	...
	/**
	 * Given an initial IEP used for bootstrapping it iterates over all of the questions and try to
	 * match the IEP to the question. If it matches, comparator pairs can be extracted from the sentence
	 * @param bootstrappingIEP
	 * @param questions 
	 * @return A list containing comparator pairs extracted from the sentence
	 */
	private List<Pair<String, String>> extractSeedComparators(String bootstrappingIEP,
			List<String> questions) {
		for(String q : questions) {
			PosTag posTag = StanfordPosTagger.posTagString(q);
			System.out.println(phraseChunker(posTag));
		}
		/*if(bootstrappingIEP.isLexical()) {
			...
			for(String q : questions) {
				...
				for(List<String> sentence : tokenized) {
					sequences.add(sentence);
				}
			}
		}
		else {
			List<Sequence> lexPat = PatternGeneration.generateLexicalPatterns(questions);
			List<Sequence> genPat = PatternGeneration.generateGeneralizedPatterns(lexPat);
			for(Sequence seq : genPat) {
				List<List<String>> tokenized = StanfordPosTagger.tokenizedString(seq.toString());
				for(List<String> sentence : tokenized) {
					if(sentence.equals(bootstrappingIEP)) {
					}
				}	
			}
		}*/
	...
	private String phraseChunker(PosTag pattern) {
		...
		for(ArrayList<TaggedWord> list : tagged) {
			...
			while(tokens.hasNext()) {
				...
				//Rule 3
				else if(word.tag().matches(rule3a.pattern()) && tokens.hasNext()) {
					TaggedWord nextWord = tokens.next(); //Peek ahead
					if(nextWord.tag().matches(rule3b.pattern())) {
						tokens.remove();
						TaggedWord current = tokens.previous(); //Move back again
						current.setValue(current.value() + " " + nextWord.value());
						current.setTag("NNS");
					}
					else {
				...
				//Rule 4
				else if(word.tag().matches(rule4a.pattern()) && tokens.hasNext()) {
				...
				//Rule 5
				else if(word.value().matches(rule5a.pattern()) && tokens.hasNext()) {
					TaggedWord nextWord = tokens.next();
				...
				//Rule 6
				else if(word.value().matches(rule6a.pattern()) && tokens.hasNext()) {
					TaggedWord nextWord = tokens.next();
